...
Bishop::Bishop(Position position, Color color) : Piece(position, color, 5)
{
}
...
void Bishop::print(){
    std::cout << ((color == WHITE) ? "b" : "B");
}

void Bishop::draw(sf::RenderWindow* window)
{
    sf::Texture texture;
    texture.loadFromFile(((color == WHITE) ? "img/white bishop.png" : "img/black bishop.png"));
    sf::Sprite sprite;
...
std::vector<Move> Bishop::getMoves(std::vector<Piece*> pieceList)
{
    Position offset[] = {{1, 1}, {-1, -1}, {-1 ,1}, {1, -1}}; // directions of movements
